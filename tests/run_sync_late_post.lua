...
-- Mage posted now (direct broadcast; B's window closed so it misses it)
wall = wall + 100
H.sentChatMessages={}
Sync.BroadcastBuild(mageB)  -- marks mageB hot
local mageChunks = drain()
deliver(mageChunks)  -- window closed; should be ignored
assert(not NexusDB.communityBuilds["m1"],
    "mage should be ignored with closed window")
...
print("build posted after window closed is received on next sync -- OK")
...
Sync.HandleIncoming("WLRQ|peer2|"..matchHash, "peer2")
