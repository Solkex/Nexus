...
-- two designed (verified=false) slots, no wishlist read via H.wishlist,
-- and NEITHER is active (activeSlot = 0)
H.DeliverSlots({
...
local activatedSlot = nil
local realActivate = Adapter.Activate
Adapter.Activate = function(slot) activatedSlot = slot; return realActivate(slot) end
...
assert(activatedSlot == 6 or activatedSlot == 7,
    "clicking a candidate button did not call Adapter.Activate with a real slot")
print("clicking a candidate button activates the correct slot -- OK")
