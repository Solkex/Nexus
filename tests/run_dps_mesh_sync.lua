...
UnitName=function() return "Relay" end; UnitLevel=function() return 80 end
local echoes={}; for i=1,79 do echoes[i]={spellId=210000+i,stacks=(i%3)+1} end
...
assert(DPS.ReceiveRecord({v=3,f=fp,e=echoes,c="dummy",d=31000000,u=65,t=50000,p="Champion",l=80,b=build.id}),"seed record failed")

H.sentChatMessages={}
clock=clock+100
Sync.HandleIncoming("WLRQ|NewPeer|0","NewPeer")
for i=1,1000 do Sync.OnUpdate(0.2) end
local sawBuild,sawDps=false,false; local dpsMessages={}
for _,m in ipairs(H.sentChatMessages) do
    ...
    if m.text:find("^WLD2|") then sawDps=true; dpsMessages[#dpsMessages+1]=m.text end
end
assert(sawBuild,"mesh response did not redistribute a remotely authored build index")
assert(sawDps,"mesh response did not include the build's highest DPS record")

-- Fresh receiver: the chunks reconstruct the exact authoritative record.
NexusDB={communityBuilds={[build.id]=build},syncTombstones={},dpsCapture={}}
DPS.Init({},Sync)
for _,text in ipairs(dpsMessages) do Sync.HandleIncoming(text,"Relay") end
...
assert(not DPS.ReceiveRecord({v=3,f=fp,e=echoes,c="dummy",d=30000000,u=65,t=50001,p="Champion",l=80,b=build.id}),"lower record should be rejected")
assert(DPS.GetLeaderboard(build.id,"dummy")[1].dps==31000000,"stale data overwrote the record")
print("DPS record mesh redistribution, chunking, reconstruction and stale rejection -- OK")
