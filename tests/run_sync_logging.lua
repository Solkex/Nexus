...
local ok, id = CB.PostCurrentWishlist("Logged Build", "desc")
...
-- RECEIVE side, failure mode 1: arriving with NO sync window open must be
-- logged with that specific reason
local msgs = {}
...
for _, m in ipairs(msgs) do
    H.FireEvent("CHAT_MSG_CHANNEL", m.text, "Bob", "Common",
        "5. " .. Sync.ChannelName(), nil, nil, nil, 5, Sync.ChannelName())
end
assert(LogHas("no sync"),
    "a build ignored due to no open sync window was not logged with that reason")
print("'ignored -- no sync requested' is logged with the exact reason -- OK")
