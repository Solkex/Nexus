...
H.Advance(14)                        -- save sends; refresh blackholed; 8s timeout -> retry
check(#H.saveCalls >= saveB4 + 2, "S22 unconfirmed save (SS-541 FAIL) retried, not falsely latched")
H.saveBlackhole = false
H.Advance(6)                         -- refresh now applies -> confirmed
local saveAfterConfirm = #H.saveCalls
H.Advance(8)
check(#H.saveCalls == saveAfterConfirm, "S22 stops retrying once the save is confirmed")
...
-- read as the target when GetActiveEchoLoadout is empty -- the real-world
-- case: users design an Echo Wishlist, they don't use "Play with..."
------------------------------------------------------------------------
...
check(wlD ~= nil and wlD.name == "My Goal" and wlD.source == "designed",
    "S26 designed Echo Wishlist build is read as the target")
check(wlD ~= nil and #wlD.entries == 3
    and wlD.byFamily["s200104"] and wlD.byFamily["s200104"].targetStacks == 3,
    "S26 designed build echoes decompose into entries (Double Strike x3 target kept)")
local chatD = #H.chat
SlashCmdList["NEXUS"]("wishlist")
check(H.ChatContains("Echo Wishlist build") ~= nil,
    "S26 /wr wishlist labels the source as the Echo Wishlist build")
...
check(H.ChatContains("TARGET:") ~= nil and H.ChatContains("Echo Wishlist build") ~= nil,
    "S27 /wr status names the target and its source")
